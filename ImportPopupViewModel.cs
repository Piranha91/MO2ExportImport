        ...
        private async void ImportMods()
        {
            ...
            try
            {
                foreach (var profile in ProfilesToImport())
                {
                    ...
                    string debugOutputDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "DebugOutput", DateTime.Now.ToString("yyyy MM dd HH mm"), profile);
                    ...
                    profileManifest.AddedPluginNames.AddRange(FormatHandler.TrimPluginActivationStatus(validPlugins));
